...
async function exportAppPackage() {
  try {
    const project = createProjectPackage();
    if (countAllWorkspaceBlocks(project.workspaces) === 0) {
      showAppMessage("Nothing to export", [
        "Add blocks to at least one Workspace tab first.",
        "Then click Export App again."
      ]);
      return;
    }

    if (!window.cyberblock || typeof window.cyberblock.exportMachineApp !== "function") {
      showAppMessage("Export App needs EXE mode", [
        "The button is now definitely connected.",
        "But Export App only works inside Electron / WorkshopTools.exe.",
        "Run npm start or open the rebuilt WorkshopTools.exe. Do not double-click index.html."
      ]);
      return;
    }

    const defaultName = localStorage.getItem("cyberblock-last-machine-name") || "Dragon Lock";
    let machineName = defaultName;
    try {
      const typedName = window.prompt("Name the exported machine/app:", defaultName);
      if (typedName === null) return;
      machineName = typedName;
    } catch (promptError) {
      // Some Electron/browser modes may block prompt. Continue with default name for testing.
      machineName = defaultName;
    }

    const cleanName = String(machineName || "Student Secret Machine").trim() || "Student Secret Machine";
    localStorage.setItem("cyberblock-last-machine-name", cleanName);

    const button = document.getElementById("exportAppBtn");
    const oldText = button ? button.textContent : "Export App";
    if (button) {
      button.disabled = true;
      button.textContent = "Exporting...";
    }

    const result = await window.cyberblock.exportMachineApp({
      machineName: cleanName,
      project
    });

    if (!result || !result.ok) {
      throw new Error(result?.message || "Export App failed.");
    }

    showAppMessage("App exported", [
      `Created folder: ${result.folderName || cleanName}`,
      `EXE file: ${result.exeName || cleanName + ".exe"}`,
      "Inside the folder, you should see the player EXE and machine.cyberblock.",
      result.outputDir ? `Location: ${result.outputDir}` : "Check your Downloads folder."
    ]);
  } catch (error) {
    showAppMessage("Export App failed", [
      error?.message || "The app package could not be created.",
      "Most common fix: make sure player-template/StudentMachinePlayer.exe exists, then run npm start again.",
      "If using the built WorkshopTools.exe, rebuild it after adding the player template."
    ]);
  } finally {
    const button = document.getElementById("exportAppBtn");
    if (button) {
      button.disabled = false;
      button.textContent = "Export App";
    }
  }
}
